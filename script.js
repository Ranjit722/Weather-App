...
api_key = 'cb5f121b8b7ddadd98d4a40aa793024c',
...
function getWeatherDetails(name, lat, lon, country, state){
    ...
    fetch(WEATHER_API_URL).then(res => res.json()).then(data => {
        ...
        currentWeatherCard.innerHTML = `
            ...
            <div class="card-footer">
                <p><i class="fas fa-calendar"></i>${days[date.getDay()]},
        ...
        {humidity, pressure, feel_like} = data.main,
        {speed} = data.wind,
        sRiseTime = moment.utc(sunrise, 'X').add(timezone, 'seconds').format('hh:mm A'),
        sSeTime = moment.utc(sunset, 'X').add(timezone, 'seconds').format('hh:mm A');
        ...
        feelsVal.innerHTML = `${(feel_like - 273.15).toFixed(2)}&deg;C`;
    }).catch(() =>{
        alert(`Failed to fetch Weather Values.`);
    })
    .finally(() => {
        hideSpinner();
    });
    ...
    fetch(FORECAST_API_URL).then(res => res.json()).then(data => {
            ...
            fiveDaysForecastCard.innerHTML += `
                <div class="forecast-item">
                    <div class="icon-wrapper">
                        ...
                        <span>${(fiveDaysForecast[i].main.temp - 237.15).toFixed(2)}&deg;C</span>
                    </div>
        ...
        alert(`Failed to fetch This Week's ForeCast.`);
    })
    .finally(() => {
        hideSpinner();
    });
